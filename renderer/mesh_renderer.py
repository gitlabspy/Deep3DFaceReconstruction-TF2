...
import tensorflow_graphics.rendering.triangle_rasterizer as triangle_rasterizer
import tensorflow_graphics as tfg

import renderer.camera_utils as camera_utils
...
def phong_shader(
    ...
    per_light_pixel_positions = tf.stack(
        [pixel_positions] * light_count, axis=1
    )  # [batch_size, light_count, pixel_count, 3]
    directions_to_lights = tf.nn.l2_normalize(
        tf.expand_dims(light_positions, axis=2) - per_light_pixel_positions, dim=3
    )  # [batch_size, light_count, pixel_count, 3]
    ...
    normals_dot_lights = tf.clip_by_value(
        tf.reduce_sum(tf.expand_dims(normals, axis=1) * directions_to_lights, axis=3),
        0.0,
        1.0,
    )  # [batch_size, light_count, pixel_count]
    diffuse_output = (
        tf.expand_dims(diffuse_colors, axis=1)
        * tf.expand_dims(normals_dot_lights, axis=3)
        * tf.expand_dims(light_intensities, axis=2)
    )
    diffuse_output = tf.reduce_sum(
        diffuse_output, axis=1
    )  # [batch_size, pixel_count, 3]
    ...
    if camera_position is not None:
        ...
        reflection_direction_dot_camera_direction = tf.reduce_sum(
            tf.expand_dims(direction_to_camera, axis=1) * mirror_reflection_direction,
            axis=3,
        )
        ...
        reflection_direction_dot_camera_direction = tf.where(
            ...
            tf.zeros_like(reflection_direction_dot_camera_direction, dtype=tf.float32),
        )
        ...
        specularity = tf.reshape(
            tf.pow(reflection_direction_dot_camera_direction, shininess_coefficients),
            [batch_size, light_count, pixel_count, 1],
        )
        specular_output = (
            tf.expand_dims(specular_colors, axis=1)
            * specularity
            * tf.expand_dims(light_intensities, axis=2)
        )
    ...
    rgb_images = tf.where(
        valid_rgb_values, rgb_images, tf.zeros_like(rgb_images, dtype=tf.float32)
    )
    return tf.reverse(
        rgb_images, axis=[1]
    )  # tf.reverse(tf.concat([rgb_images, alpha_images], axis=3), axis=[1])
...
def tone_mapper(image, gamma):
    ...
    image_max = tf.reduce_max(tf.reshape(corrected_image, [batch_size, -1]), axis=1)
    scaled_image = tf.divide(
        corrected_image, tf.reshape(image_max, [batch_size, 1, 1, 1])
    )
...
def mesh_renderer(
    ...
    if ambient_color is not None and ambient_color.get_shape().as_list() != [
        batch_size,
        3,
    ]:
    ...
    if camera_lookat.get_shape().as_list() == [3]:
        camera_lookat = tf.tile(tf.expand_dims(camera_lookat, axis=0), [batch_size, 1])
    ...
    if isinstance(fov_y, float):
        ...
        raise ValueError(
            "Fov_y must be a float, a 0D tensor, or a 1D tensor with"
            "shape [batch_size]"
        )
    if isinstance(near_clip, float):
        ...
        raise ValueError(
            "Near_clip must be a float, a 0D tensor, or a 1D tensor"
    ...
    if isinstance(far_clip, float):
        ...
        raise ValueError(
            "Far_clip must be a float, a 0D tensor, or a 1D tensor"
            "with shape [batch_size]"
        )
    ...
    if specular_colors is not None:
        ...
        if len(specular_colors.shape) != 3:
            raise ValueError(
                "The specular colors must have shape [batch_size, " "vertex_count, 3]."
            )
        ...
        if len(shininess_coefficients.shape) < 2:
            ...
            vertex_attributes = {
                ...
                "shininess_coefficients": tf.expand_dims(
                    shininess_coefficients, axis=2
                ),
            }
    else:
        vertex_attributes = {
    ...
    out = triangle_rasterizer.rasterize(
        vertices,
        triangles,
        vertex_attributes,
        clip_space_transforms,
        (image_width, image_height),
        # backend=tfg.rendering.rasterization_backend.RasterizationBackends.CPU,
    )

    pixel_normals = tf.nn.l2_normalize(out["normals"], dim=3)
    pixel_positions = out["vertices"]
    diffuse_colors = out["diffuse_colors"]

    if specular_colors is not None:
        specular_colors = out["specular_colors"]  # pixel_attributes[:, :, :, 9:12]
        # Retrieve the interpolated shininess coefficients if necessary, or just
        # reshape our input for broadcasting:
        if len(shininess_coefficients.shape) == 2:
            shininess_coefficients = out[
    ...
    renders = phong_shader(
        ...
        alphas=tf.squeeze(out["mask"], axis=-1),  # alphas,
